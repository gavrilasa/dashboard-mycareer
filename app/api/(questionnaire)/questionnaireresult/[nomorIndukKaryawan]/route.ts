...
export async function GET(req: NextRequest, {params}:{params:{nomorIndukKaryawan: string}}) {
    ...
    try {
        ...
        const questionnaireResult = await prisma.$transaction( async (tx) => {
            const idResponses: Array<{idResp: string, id_form: string}> = await tx.responses.findMany({
                where: {
                    nomorIndukKaryawan: nomorIndukKaryawan,
                },
                select: {
            ...
            const forms: Array<null> | Array<object> | Array<object | null> = await Promise.all(
                idResponses.map((e: {idResp: string, id_form: string})=>tx.forms.findUnique({
                    where: {
                        idForm: e.id_form
                    },
                    include:{
            ...
            const answers: Array<Array<{
                ...
                idResponses.map((e: {idResp: string, id_form: string})=>tx.answers.findMany({
                    where: {
                        idResp: e.idResp,
                    },
                    select: {
            ...
            return forms.filter((e:any)=>e!==null).map((form:any)=>{
                return {
                    IDForm: form.idForm,
                    ...
                    AssessmentType: form.AssessmentType.map((assessmen: any) => ({
                        ...
                        Ques: assessmen.Questions.map((q: any) => ({
                            ...
                            TitleQue: q.titleQue,
                            DescQue: q.Question,
                            answer: flatAnswers.find((e:any)=> e.idAssess === assessmen.idAssessmentType && e.idQuestion === q.idQuestion)?.answer,
                        })),
                    })),
                }
            })
