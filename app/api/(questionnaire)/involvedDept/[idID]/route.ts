...
export async function DELETE(req: NextRequest, { params } : { params : { idID: string }}) {
    ...
    const { idID } = params;
    try {
        const deletedInvolvedDept = await prisma.involvedDept.delete({
            where: { idID: idID }
        });
        return NextResponse.json(deletedInvolvedDept, { status: 200 });
    } catch (error) {
