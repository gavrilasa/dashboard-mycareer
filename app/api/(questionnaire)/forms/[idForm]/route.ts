...
export async function GET(req: NextRequest, { params }: { params: { idForm: string } }) {
    ...
    if (session.user.role !== "hr" && session.user.branch !== "N001") {
        return NextResponse.json(
            { error: "Forbidden", message: "Anda bukanlah seorang HR dari Head Office. Data ini tidak bisa anda akses."},
            { status: 403 }
        );
    }
