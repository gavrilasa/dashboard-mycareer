...
export async function DELETE(req: NextRequest, {params}:{params: {idPos: string}}){
    ...
    const idPos = await params;

    if (!idPos) {
        return NextResponse.json({ error: "Bad Request", message: "ID Position tidak ditemukan" }, { status: 400 });
    }

    try {
        const result = await prisma.dataPosition.delete({
            where: {
                idPosition: parseInt(idPos.idPos),
            } 
        })
