...
export async function PUT(
	...
	{
		params,
	}: { params: { nomorIndukKaryawan: string; idCareerHistory: string } }
) {
...
export async function DELETE(
	...
	try {
		...
		if (!dataRiwayatKarir) {
			return NextResponse.json(
				{ error: "Data not found", message: "Data tidak ditemukan!" },
				{ status: 404 }
			);
		}
			...
			return NextResponse.json(
				{
					error: "Bad request",
					message:
						"Tidak bisa menghapus posisi aktif anda sekarang! Buat posisi aktif baru terlebih dahulu!",
				},
				{ status: 400 }
			);
		}
