...
export async function GET(
	...
	try {
		const dataRiwayatOrganisasiInternal =
			await prisma.dataRiwayatOrganisasiInternal.findMany({
				where: {
					nomorIndukKaryawan: nomorIndukKaryawan,
				},
			});
		if (!dataRiwayatOrganisasiInternal.length) {
			return NextResponse.json(
				{
...
export async function POST(
	...
	if (session.user.role === "hd") {
		return NextResponse.json(
			{
				error: "Unauthorized",
				message: "Anda tidak memiliki akses untuk melakukan task ini!",
			},
			{ status: 401 }
		);
	}
	...
	try {
		...
		const dataRiwayatOrganisasiInternal =
			await prisma.dataRiwayatOrganisasiInternal.create({
				data: {
					nomorIndukKaryawan: nomorIndukKaryawan,
