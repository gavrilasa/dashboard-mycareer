...
export async function PUT(req: NextRequest, { params }:{params:{nomorIndukKaryawan: string}}){
    ...
    try{
        ...
        const isRiwayatKarirUpdated = prevData && data.personnelArea === prevData.personnelArea && data.personnelSubarea === prevData.personnelSubarea && String(data.position) === String(prevData.position) && data.levelPosition === prevData.levelPosition;
        let result;
