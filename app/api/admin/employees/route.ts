...
export async function GET() {
	...
	if (!session || !session.user) {
		return NextResponse.json(
			{ error: "Unauthorized", message: "Anda tidak terdaftar" },
			{ status: 401 }
		);
	}
	...
	try {
		...
		const returnedData = usersWithEmployeeData.map((e: any) => {
			return {
				...
				personnelSubarea:
					e.DataKaryawan.DataPosition.DataDepartment.namaDepartment,
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		try {
			...
			return NextResponse.json(
				{ error: "Error parsing form data", message: (error as any).message },
				{ status: 400 }
			);
		}
		...
		if (!file)
			return NextResponse.json(
				{ error: "File not found", message: "No file uploaded" },
				{ status: 400 }
			);
			...
			for (let index = 0; index < jsonData.length; index++) {
				...
				if (element[2] !== session.user.branch) {
					return NextResponse.json(
						{
							error: "unauthorized",
							message: `Silahkan mengupload data khusus untuk karyawan cabang anda, dalam data terdapat karyawan cabang lain dalam baris ${
								index + 2
							} `,
						},
						{ status: 401 }
		...
		const DBInput_User = DBInput.map((e: any) => {
			return {
				...
				password: generatePassword(
					String(e[3]),
					convertDate(e[9]).toISOString().split("T")[0]
				),
				role: "employee",
		...
		const DBInput_DataKaryawan = await Promise.all(
			DBInput.map(async (e: any) => {
				return {
					nomorIndukKaryawan: String(e[3]),
		...
		const DBInput_DataRiwayatKarir = await Promise.all(
			DBInput.map(async (e: any) => {
				return {
					nomorIndukKaryawan: String(e[3]),
					position: (await dbAligner.toIDPos(String(e[5]), String(e[6]))) ?? 0,
