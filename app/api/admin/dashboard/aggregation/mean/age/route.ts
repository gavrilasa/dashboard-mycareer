...
export async function GET(req: NextRequest) {
    ...
    const personnelArea = searchParams.get("personnelArea") as string ?? "Semua Cabang";
    const personnelSubarea = searchParams.get("personnelSubarea") as string ?? "Semua Department";
    const levelPosition = searchParams.get("levelPosition") as string ?? "Semua Level";
    try {
        ...
        const sumAge = sumResult._sum.age ?? 0;
        const countResult = await countData()
        const count = countResult;

        dataReturned = count !== 0 ? parseFloat((sumAge / count).toFixed(2)) : 0;

        console.log("ini adalah data return unutk rata2 usia",countData,sumAge,sumResult, dataReturned);
