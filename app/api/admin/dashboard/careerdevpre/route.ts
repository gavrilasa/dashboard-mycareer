...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const totalEmployees = await prisma.dataKaryawan.count({
            where: {
                personnelArea: area,
        ...
        const totalEmployees_formFilled= await prisma.dataKaryawan.count({
            where: {
                personnelArea: area,
                personnelSubarea: subarea,
                levelPosition: level,
        ...
        const totalEmployees_careerDevPreFilled = await prisma.dataKaryawan.findMany({
            where: {
                personnelArea: area,
                personnelSubarea: subarea,
                levelPosition: level,
        ...
        const filteredCareerDevPre = totalEmployees_careerDevPreFilled.filter(emp => emp.EmpCareerChoice !== null);
        const careerDev1 = filteredCareerDevPre.filter(emp => emp.EmpCareerChoice?.careerDevWill === true).length;
        const crossDept1 = filteredCareerDevPre.filter(emp => emp.EmpCareerChoice?.crossDeptWill === true).length;
        const rotation1 = filteredCareerDevPre.filter(emp => emp.EmpCareerChoice?.rotationWill === true).length;
        
        const dataReturned = [
            {
                name: "Total Karyawan", 
                pos: { value: totalEmployees, label: "Total" },
                neg: { value: 0, label: "-" }
            },
            {
                name: "Form Terisi", 
                pos: { value: totalEmployees_formFilled, label: "Terisi" },
                neg: { value: totalEmployees - totalEmployees_formFilled, label: "Belum Terisi" },
            },
            {
                name: "Career Development", 
                pos: { value: careerDev1, label: "Bersedia" },
                neg: { value: totalEmployees_formFilled - careerDev1, label: "Tidak Bersedia" }
            },
            {
                name: "Cross Department", 
                pos: { value: crossDept1, label: "Bersedia" }, 
                neg: { value: totalEmployees_formFilled - crossDept1, label: "Tidak Bersedia" }
            },
            {
                name: "Rotation", 
                pos: { value: rotation1, label: "Bersedia" },
                neg: { value: totalEmployees_formFilled - rotation1, label: "Tidak Bersedia" },
            }
        ]
