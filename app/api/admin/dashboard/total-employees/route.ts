...
export async function GET(req: NextRequest) {
	...
	try {
		...
		if (groupby) {
			if (
				Object.values(Prisma.DataKaryawanScalarFieldEnum).includes(
					groupby as Prisma.DataKaryawanScalarFieldEnum
				)
			) {
		...
		const finishedData = totEmp.map((item) => ({
			name:
				groupby === "personnelArea"
					? branch.find(
							(e) => e.idBranch === (item[gb[0] as keyof typeof item] as string)
					  )?.namaBranch
					: groupby === "personnelSubarea"
					? dept.find(
							(e) =>
								e.idDepartment === (item[gb[0] as keyof typeof item] as string)
					  )?.namaDepartment
					: (item[gb[0] as keyof typeof item] as string),
