...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        let level = undefined;
        let gb: Prisma.DataKaryawanScalarFieldEnum[] | undefined = undefined;

        if (groupby) {
            if (Object.values(Prisma.DataKaryawanScalarFieldEnum).includes(groupby as Prisma.DataKaryawanScalarFieldEnum)) {
                gb = [groupby as Prisma.DataKaryawanScalarFieldEnum];
            } else {
        ...
        const totEmp = await prisma.dataKaryawan.groupBy({
            by: gb,
            where: {
                personnelArea: area,
        ...
        const branch = await prisma.dataBranch.findMany();
        const dept = await prisma.dataDepartment.findMany();
        const finishedData = totEmp.map((item) => ({
            name: groupby === "personnelArea" ? branch.find((e)=>e.idBranch === item[gb[0] as keyof typeof item] as string)?.namaBranch : groupby === "personnelSubarea" ? dept.find((e)=>e.idDepartment === item[gb[0] as keyof typeof item] as string)?.namaDepartment : item[gb[0] as keyof typeof item] as string,
            value: item._count._all,
            fill: null
        }));

        return NextResponse.json({ data: finishedData }, { status: 200 });
    } catch (error) {
