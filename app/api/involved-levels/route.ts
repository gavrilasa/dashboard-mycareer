...
export async function GET(req: NextRequest) {
	...
	try {
		const involvedLevel = await prisma.involvedLevel.findMany({});
		return NextResponse.json(involvedLevel, { status: 200 });
	} catch (error) {
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const createdInvolvedLevel = await prisma.involvedLevel.createMany({
			data: data,
		});

		return NextResponse.json(createdInvolvedLevel, { status: 200 });
	} catch (error) {
		if (error instanceof Prisma.PrismaClientKnownRequestError) {
			return NextResponse.json(
				{
