...
export async function GET(req: NextRequest, {params}:{params:{nomorIndukKaryawan: string}}) {
    ...
    try{
        let dashboardKaryawan = await prisma.dataKaryawan.findUnique({
            where: {
        ...
        console.log("DATAKARYAWAN", dashboardKaryawan);
        return NextResponse.json(dashboardKaryawan, { status: 200 });
    } catch (error) {
